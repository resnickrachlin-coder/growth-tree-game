  ...
  while ((match = regex.exec(mdContent)) !== null) {
    try {
      ...
      try {
        const jsCode = match[1].trim();
        // 确保对象键有引号
        let cleaned = jsCode.replace(/([{,]\s*)(\w+)(\s*:)/g, '$1"$2"$3');
        // 处理单引号
        cleaned = cleaned.replace(/'/g, '"');
        const obj = JSON.parse(cleaned);
        blocks.push(obj);
      } catch(e2) {
        console.error('Failed to parse:', e2.message.substring(0, 80));
      }
    }
  }
  return blocks;
...
for (const file of files) {
  ...
  console.log(`${file}: ${models.length} models parsed successfully`);
}
...
console.log(`\n总解析模型: ${totalModels}`);
console.log(`总章节: ${allModels.length}`);
...
console.log('Saved to converted_chapters.json');
